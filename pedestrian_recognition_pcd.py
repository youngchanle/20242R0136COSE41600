...
def process_pcd_files_with_boxes(pcd_file_path_1, pcd_file_path_2, output_file_path):
    ...
    tree = cKDTree(points2)  # Change to cKDTree
    ...
    for i in range(len(points1)):
        dist, _ = tree.query([points1[i]], k=1)  # Query nearest neighbor
        if dist <= threshold:  # Directly check distance
            mask_to_remove[i] = True

    # Remaining points after filtering
    filtered_points = points1[~mask_to_remove]

    # Perform 3D clustering directly on the filtered points
    eps_3d = 0.5  # Adjust clustering parameters as needed
    min_samples_3d = 10
    clustering_3d = DBSCAN(eps=eps_3d, min_samples=min_samples_3d).fit(filtered_points)
    labels_3d = clustering_3d.labels_
    unique_labels_3d = set(labels_3d)

    boxes_info = []
    colors = np.random.rand(len(unique_labels_3d), 3)  # Generate random colors for visualization

    # Process each 3D cluster
    for label_3d in unique_labels_3d:
        if label_3d == -1:
            continue  # Skip noise points

        cluster_indices_3d = (labels_3d == label_3d)
        cluster_points_3d = filtered_points[cluster_indices_3d]
        ...
        # Only process boxes with height >= 0.5
        if bbox_height >= 0.5:
...
pcd_file_path_1 = "/Users/iyeongchan/Desktop/Projrct/COSE416_HW1_tutorial/data/05_straight_duck_walk/pcd/pcd_000456.pcd"
pcd_file_path_2 = "/Users/iyeongchan/Desktop/Projrct/COSE416_HW1_tutorial/data/05_straight_duck_walk/pcd/pcd_000056.pcd"
output_file_path = "/Users/iyeongchan/Desktop/Projrct/COSE416_HW1_tutorial/data/01_straight_walk/hi.json"

process_pcd_files_with_boxes(pcd_file_path_1, pcd_file_path_2, output_file_path)
...
print(f"경과 시간: {elapsed_time_in_minutes:.2f} 분")
